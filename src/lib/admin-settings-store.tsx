...
export const PERMISSION_GROUPS = [
  "Dashboard", "Employees", "EMS", "Attendance", "Leave/WFH", "Projects",
  "Team Utilization", "Timesheet", "Expense Management", "Assets", "Helpdesk",
...
export type WorkflowKey = "leave" | "wfh" | "timesheet" | "expense" | "asset_request" | "helpdesk_escalation";
...
export interface PolicyAttendance {
  graceMinutes: number; halfDayAfterMinutes: number; autoMarkAbsentMinutes: number; allowRegularization: boolean;
}
export interface PolicyLeave {
  casualPerYear: number; sickPerYear: number; earnedPerYear: number; carryForwardCap: number; encashmentAllowed: boolean;
}
export interface PolicyTimesheet {
  weeklyHours: number; minDailyHours: number; submitBy: string; lockAfterApproval: boolean;
}
export interface PolicyExpense {
  perDayLimit: number; receiptMandatoryAbove: number; selfApprovalAllowed: boolean; autoEscalateDays: number;
}
export interface PolicyAsset {
  damagePenalty: boolean; mandatoryAck: boolean; returnSlaDays: number; warrantyAlertDays: number;
}
export interface PolicySLA {
  urgentResponseHrs: number; urgentResolveHrs: number; highResponseHrs: number; highResolveHrs: number;
  normalResponseHrs: number; normalResolveHrs: number; lowResponseHrs: number; lowResolveHrs: number;
}
...
const toMaster = (id: string, name: string, meta?: string): MasterRow => ({
  id, name, active: true, meta,
});

const DEFAULT_MASTERS: MasterData = {
  departments: SEED_DEPTS.map((d) => ({ id: d.id, name: d.name, active: true, meta: `Head: ${d.head} • ${d.headcount} ppl` })),
  designations: SEED_DSGN.map((d) => ({ id: d.id, name: d.title, active: true, meta: `${d.level} • ${d.department}` })),
  employmentTypes: [
    toMaster("ET-01", "Full-time"), toMaster("ET-02", "Part-time"),
    toMaster("ET-03", "Intern"), toMaster("ET-04", "Contractor"), toMaster("ET-05", "Consultant"),
  ],
  workLocations: [
    toMaster("WL-01", "Bengaluru HQ", "India"), toMaster("WL-02", "Mumbai Office", "India"),
    toMaster("WL-03", "Singapore", "APAC"), toMaster("WL-04", "Remote — India", "Distributed"),
  ...
  leaveTypes: [
    toMaster("LT-01", "Casual Leave", "12/year"), toMaster("LT-02", "Sick Leave", "10/year"),
    toMaster("LT-03", "Earned Leave", "18/year"), toMaster("LT-04", "Maternity", "26 weeks"),
    toMaster("LT-05", "Paternity", "10 days"), toMaster("LT-06", "Bereavement", "5 days"),
    toMaster("LT-07", "Comp-off", "On approval"),
  ],
  expenseCategories: [
    toMaster("EC-01", "Travel"), toMaster("EC-02", "Accommodation"), toMaster("EC-03", "Food"),
    toMaster("EC-04", "Client Entertainment"), toMaster("EC-05", "Software / Subscription"),
    toMaster("EC-06", "Hardware"), toMaster("EC-07", "Training"), toMaster("EC-08", "Misc"),
  ],
  assetCategories: [
    toMaster("AC-01", "Laptop"), toMaster("AC-02", "Desktop"), toMaster("AC-03", "Monitor"),
    toMaster("AC-04", "Mobile Phone"), toMaster("AC-05", "Accessory"), toMaster("AC-06", "Software License"),
  ],
  helpdeskCategories: [
    toMaster("HC-01", "IT"), toMaster("HC-02", "HR"), toMaster("HC-03", "Finance"),
    toMaster("HC-04", "Admin"), toMaster("HC-05", "Assets"), toMaster("HC-06", "Project Support"),
  ],
  projectRoles: [
    toMaster("PR-01", "Project Manager"), toMaster("PR-02", "Tech Lead"),
    toMaster("PR-03", "Developer"), toMaster("PR-04", "QA Engineer"),
    toMaster("PR-05", "Designer"), toMaster("PR-06", "Business Analyst"),
  ],
};
...
function defaultMatrixFor(roleKey: string): RoleConfig["matrix"] {
  const m = emptyMatrix();
  const set = (g: PermissionGroup, acts: PermissionAction[]) => { acts.forEach((a) => (m[g][a] = true)); };
  const all: PermissionAction[] = ["view", "create", "edit", "delete", "approve", "export", "configure"];
  const ro: PermissionAction[] = ["view", "export"];

  if (roleKey === "main_admin") { for (const g of PERMISSION_GROUPS) set(g, all); return m; }
  if (roleKey === "auditor") { for (const g of PERMISSION_GROUPS) set(g, ro); return m; }
...
const ASSIGNED_COUNT_OVERRIDES: Record<string, number> = {
  main_admin: 2, hr_admin: 4, admin: 3, employee: 168, manager: 14, team_lead: 12,
  module_lead: 9, project_manager: 7, finance_manager: 3, asset_admin: 2, helpdesk_agent: 5, auditor: 1,
};
...
function buildDefaultRoles(): RoleConfig[] {
  ...
  const extras: RoleConfig[] = EXTRA_ROLE_KEYS.map((k) => ({
    key: k,
    label: k.split("_").map((s) => s[0].toUpperCase() + s.slice(1)).join(" "),
...
const stage = (n: number, type: WorkflowStage["approverType"], val: string, esc = 2): WorkflowStage => ({
  id: `s_${n}_${Math.random().toString(36).slice(2, 7)}`, approverType: type, approverValue: val, escalateAfterDays: esc, mandatoryRemarksOnReject: true,
});

const DEFAULT_WORKFLOWS: WorkflowConfig[] = [
  { key: "leave", label: "Leave approval", active: true, stages: [stage(1, "Reporting Manager", "Direct manager", 2), stage(2, "Role", "HR Admin", 3)] },
  { key: "wfh", label: "WFH approval", active: true, stages: [stage(1, "Reporting Manager", "Direct manager", 1)] },
  { key: "timesheet", label: "Timesheet approval", active: true, stages: [stage(1, "Reporting Manager", "Direct manager", 2), stage(2, "Role", "Project Manager", 2)] },
  { key: "expense", label: "Expense approval", active: true, stages: [stage(1, "Reporting Manager", "Direct manager", 2), stage(2, "Role", "Director / HoD", 2), stage(3, "Role", "Finance Manager", 3)] },
  { key: "asset_request", label: "Asset request approval", active: true, stages: [stage(1, "Reporting Manager", "Direct manager", 2), stage(2, "Role", "IT / Asset Admin", 2)] },
  { key: "helpdesk_escalation", label: "Helpdesk escalation", active: true, stages: [stage(1, "Role", "Helpdesk Agent", 1), stage(2, "Role", "Module Owner", 1), stage(3, "Specific User", "Aanya Mehta (Main Admin)", 1)] },
];

const DEFAULT_POLICIES: Policies = {
  attendance: { graceMinutes: 10, halfDayAfterMinutes: 240, autoMarkAbsentMinutes: 480, allowRegularization: true },
  leave: { casualPerYear: 12, sickPerYear: 10, earnedPerYear: 18, carryForwardCap: 30, encashmentAllowed: true },
  timesheet: { weeklyHours: 40, minDailyHours: 6, submitBy: "Monday 11:00 AM", lockAfterApproval: true },
  expense: { perDayLimit: 5000, receiptMandatoryAbove: 500, selfApprovalAllowed: false, autoEscalateDays: 3 },
  asset: { damagePenalty: true, mandatoryAck: true, returnSlaDays: 5, warrantyAlertDays: 60 },
  sla: { urgentResponseHrs: 1, urgentResolveHrs: 4, highResponseHrs: 4, highResolveHrs: 24, normalResponseHrs: 8, normalResolveHrs: 48, lowResponseHrs: 24, lowResolveHrs: 96 },
};

const DEFAULT_TEMPLATES: EmailTemplate[] = [
  { key: "invite", name: "Employee Invite", subject: "Welcome to {{company}} — set up your account", body: "Hi {{name}},\n\nYou've been invited to join {{company}} on Hawkaii. Click the link below to set your password and complete onboarding.\n\n{{link}}\n\n— People Ops", active: true },
  { key: "verify", name: "Email Verification", subject: "Verify your email for {{company}}", body: "Hi {{name}},\n\nPlease verify your email by clicking the link below.\n\n{{link}}\n\nThis link expires in 24 hours.", active: true },
  { key: "reset", name: "Password Reset", subject: "Reset your {{company}} password", body: "Hi {{name}},\n\nUse the link below to reset your password. If you didn't request this, you can ignore this email.\n\n{{link}}", active: true },
  { key: "leave", name: "Leave Approval", subject: "Your leave request was {{status}}", body: "Hi {{name}},\n\nYour leave request from {{from}} to {{to}} has been {{status}} by {{approver}}.\n\nRemarks: {{remarks}}", active: true },
  { key: "expense", name: "Expense Approval", subject: "Expense {{id}} {{status}}", body: "Hi {{name}},\n\nYour expense claim {{id}} for {{amount}} has been {{status}}.\n\nRemarks: {{remarks}}", active: true },
  { key: "ts_reminder", name: "Timesheet Reminder", subject: "Submit your weekly timesheet", body: "Hi {{name}},\n\nFriendly reminder to submit your timesheet for week ending {{week}} before {{deadline}}.", active: true },
  { key: "ticket_update", name: "Helpdesk Ticket Update", subject: "Ticket {{id}} updated — {{status}}", body: "Hi {{name}},\n\nTicket {{id}} ({{title}}) is now {{status}}.\n\nLatest update: {{message}}", active: true },
];
...
const DEFAULT_NOTIFICATIONS: NotificationEvent[] = [
  ...
  { key: "timesheet_submitted", label: "Timesheet submitted", inApp: true, email: false, push: false },
  ...
  { key: "ticket_assigned", label: "Helpdesk ticket assigned", inApp: true, email: true, push: false },
  { key: "sla_breached", label: "SLA breached", inApp: true, email: true, push: true },
];

const DEFAULT_SECURITY: SecuritySettings = {
  passwordMinLength: 10, passwordRequireSpecial: true, passwordRequireNumber: true,
  passwordExpiryDays: 90, sessionTimeoutMinutes: 60, mfaEnabled: false,
  loginAttemptLimit: 5, auditRoleChanges: true, ipDeviceAuditEnabled: true,
};

const EXTRA_AUDIT: AuditLogEntry[] = [
  { id: "AL-1006", actor: "Aanya Mehta", action: "updated.policy", target: "expense.perDayLimit 4000→5000", module: "Admin Settings", at: "2026-05-09 14:02", ip: "10.0.4.12" },
  { id: "AL-1007", actor: "Rahul Verma", action: "updated.role", target: "hr_admin permissions", module: "Admin Settings", at: "2026-05-08 16:11", ip: "10.0.4.18" },
  { id: "AL-1008", actor: "Aanya Mehta", action: "added.master", target: "Work Location: Singapore", module: "Admin Settings", at: "2026-05-07 10:55", ip: "10.0.4.12" },
  { id: "AL-1009", actor: "Marco Rossi", action: "updated.workflow", target: "expense — added Director stage", module: "Admin Settings", at: "2026-05-06 11:42", ip: "10.0.4.27" },
  { id: "AL-1010", actor: "Aanya Mehta", action: "rotated.security", target: "passwordExpiryDays 60→90", module: "Admin Settings", at: "2026-05-05 09:18", ip: "10.0.4.12" },
];
...
interface Ctx {
  ...
  updateRoleMeta: (roleKey: string, patch: Partial<Pick<RoleConfig, "label" | "description">>) => void;
...
export function AdminSettingsProvider({ children }: { children: React.ReactNode }) {
  ...
  const [notifications, setNotifications] = React.useState<NotificationEvent[]>(DEFAULT_NOTIFICATIONS);
  ...
  const log: Ctx["log"] = (entry) => {
    ...
    setAudit((prev) => { const n = [next, ...prev]; ls.set(K.audit, n); return n; });
  };

  const setCompany: Ctx["setCompany"] = (c) => { setCompanyState(c); ls.set(K.company, c); log({ actor: "Aanya Mehta", action: "updated.company", target: c.name, module: "Admin Settings" }); };

  const addMaster: Ctx["addMaster"] = (k, name, description) => {
    setMasters((prev) => {
      const row: MasterRow = { id: uid(k.slice(0, 2).toUpperCase()), name, description, active: true };
      const n = { ...prev, [k]: [row, ...prev[k]] };
      ls.set(K.masters, n); return n;
    });
    log({ actor: "Aanya Mehta", action: "added.master", target: `${k}: ${name}`, module: "Admin Settings" });
  };
  const updateMaster: Ctx["updateMaster"] = (k, id, patch) => setMasters((prev) => {
    const n = { ...prev, [k]: prev[k].map((r) => r.id === id ? { ...r, ...patch } : r) };
    ls.set(K.masters, n); return n;
  });
  const toggleMasterActive: Ctx["toggleMasterActive"] = (k, id) => setMasters((prev) => {
    const n = { ...prev, [k]: prev[k].map((r) => r.id === id ? { ...r, active: !r.active } : r) };
    ls.set(K.masters, n); return n;
  });
  const deleteMaster: Ctx["deleteMaster"] = (k, id) => setMasters((prev) => {
    const n = { ...prev, [k]: prev[k].filter((r) => r.id !== id) };
    ls.set(K.masters, n); return n;
  });

  const persistRoles = (n: RoleConfig[]) => { setRoles(n); ls.set(K.roles, n); };
  const togglePermission: Ctx["togglePermission"] = (roleKey, group, action) => {
    persistRoles(roles.map((r) => r.key === roleKey
      ? { ...r, matrix: { ...r.matrix, [group]: { ...r.matrix[group], [action]: !r.matrix[group][action] } } }
      : r));
  };
  ...
  const persistWf = (n: WorkflowConfig[]) => { setWorkflows(n); ls.set(K.workflows, n); };
  const toggleWorkflow: Ctx["toggleWorkflow"] = (k) => persistWf(workflows.map((w) => w.key === k ? { ...w, active: !w.active } : w));
  const addStage: Ctx["addStage"] = (k) => persistWf(workflows.map((w) => w.key === k
    ? { ...w, stages: [...w.stages, stage(w.stages.length + 1, "Role", "Manager", 2)] } : w));
  const updateStage: Ctx["updateStage"] = (k, stageId, patch) => persistWf(workflows.map((w) => w.key === k
    ? { ...w, stages: w.stages.map((s) => s.id === stageId ? { ...s, ...patch } : s) } : w));
  const removeStage: Ctx["removeStage"] = (k, stageId) => persistWf(workflows.map((w) => w.key === k
    ? { ...w, stages: w.stages.filter((s) => s.id !== stageId) } : w));

  const setPolicy: Ctx["setPolicy"] = (k, patch) => setPolicies((prev) => {
    const n = { ...prev, [k]: { ...prev[k], ...patch } };
    ls.set(K.policies, n); return n;
  });

  const updateTemplate: Ctx["updateTemplate"] = (key, patch) => setTemplates((prev) => {
    const n = prev.map((t) => t.key === key ? { ...t, ...patch } : t);
    ls.set(K.templates, n); return n;
  });

  const toggleNotification: Ctx["toggleNotification"] = (key, channel) => setNotifications((prev) => {
    const n = prev.map((e) => e.key === key ? { ...e, [channel]: !e[channel] } : e);
    ls.set(K.notifications, n); return n;
  });

  const setSecurity: Ctx["setSecurity"] = (patch) => setSecurityState((prev) => {
    const n = { ...prev, ...patch }; ls.set(K.security, n); return n;
  });

  const value: Ctx = {
    company, setCompany,
    masters, addMaster, updateMaster, toggleMasterActive, deleteMaster,
    roles, togglePermission, toggleAllForGroup, updateRoleMeta,
    workflows, toggleWorkflow, addStage, updateStage, removeStage,
    policies, setPolicy,
