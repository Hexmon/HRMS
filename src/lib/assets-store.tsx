...
export function AssetsProvider({ children }: { children: React.ReactNode }) {
  ...
  const persistA = (next: Asset[]) => { setAssets(next); ls.set(A_KEY, next); };
  const persistR = (next: AssetRequest[]) => { setRequests(next); ls.set(R_KEY, next); };
  ...
  const updateAsset: Ctx["updateAsset"] = (id, patch, actor = "Marco Rossi", action = "Asset updated") => {
    persistA(assets.map((x) => x.id === id ? { ...x, ...patch, audit: [audit(actor, action), ...x.audit] } : x));
  };

  const deleteAsset: Ctx["deleteAsset"] = (id) => persistA(assets.filter((x) => x.id !== id));

  const assignAsset: Ctx["assignAsset"] = (id, employee, employeeId, assignedOn, expectedReturn, condition, remarks, actor = "Marco Rossi") => {
    persistA(assets.map((x) => {
      if (x.id !== id) return x;
      const entry: AssignmentEntry = {
        id: "h_" + Math.random().toString(36).slice(2, 8),
        employee, employeeId, assignedOn, conditionAtHandover: condition, remarks, acknowledged: false,
      };
      return {
        ...x,
        status: "assigned",
        assignedTo: employee, assignedToId: employeeId, assignedOn, expectedReturn,
  ...
  const returnAsset: Ctx["returnAsset"] = (id, returnedOn, conditionAtReturn, remarks, actor = "Marco Rossi") => {
    persistA(assets.map((x) => {
      if (x.id !== id) return x;
      const [latest, ...rest] = x.history;
      const updatedLatest = latest ? { ...latest, returnedOn, conditionAtReturn, remarks: remarks || latest.remarks } : latest;
      return {
        ...x,
        status: "available", assignedTo: undefined, assignedToId: undefined, assignedOn: undefined, expectedReturn: undefined,
  ...
  const addMaintenance: Ctx["addMaintenance"] = (id, entry, actor = "Marco Rossi") => {
    persistA(assets.map((x) => x.id === id ? {
      ...x, maintenance: [entry, ...x.maintenance],
  ...
  const decideRequest: Ctx["decideRequest"] = (id, status, by, remarks) => {
    persistR(requests.map((r) => r.id === id ? {
      ...r, status, decisionBy: by, decisionAt: new Date().toISOString(), decisionRemarks: remarks,
    } : r));
  };

  const cancelRequest: Ctx["cancelRequest"] = (id) => persistR(requests.filter((r) => r.id !== id));

  const reset = () => { persistA(SEED_ASSETS); persistR(SEED_REQS); };

  return (
    <Ctx_.Provider value={{
      assets, requests,
      addAsset, updateAsset, deleteAsset,
      assignAsset, acknowledgeAssignment, returnAsset, setStatus, addMaintenance,
      addRequest, decideRequest, cancelRequest, reset,
    }}>
      {children}
    </Ctx_.Provider>
  );
