...
export const Route = createFileRoute("/_app/expenses/review")({ component: ReviewerQueue });
...
function ReviewerQueue() {
  ...
  const groups = useMemo(() => ({
    pending: tickets.filter((t) => t.status === "pending_reviewer"),
    approved: tickets.filter((t) => t.approvals.some((a) => a.role === "Reviewer" && a.status === "approved") && t.status !== "reviewer_returned"),
    returned: tickets.filter((t) => t.status === "reviewer_returned"),
    rejected: tickets.filter((t) => t.status === "reviewer_rejected"),
  }), [tickets]);

  return (
    <Tabs defaultValue="pending">
      <TabsList>
        <TabsTrigger value="pending">Pending review ({groups.pending.length})</TabsTrigger>
      ...
      {(["pending", "approved", "returned", "rejected"] as const).map((k) => (
        <TabsContent key={k} value={k} className="mt-4">
          <DataTable columns={cols} rows={groups[k]} searchKeys={["id", "employee", "taskTitle"]}
            emptyTitle="Queue clear" emptyDescription="No tickets in this state." />
        </TabsContent>
      ))}
    </Tabs>
  );
