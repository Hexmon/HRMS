...
function HolidaysPage() {
  ...
  const [name, setName] = useState(""); const [date, setDate] = useState(""); const [region, setRegion] = useState(""); const [optional, setOptional] = useState(false);
  ...
  const monthHolidays = useMemo(() => list.filter((h) => {
    const d = new Date(h.date); return d.getFullYear() === cursor.y && d.getMonth() === cursor.m;
  }), [list, cursor]);

  const add = () => {
    if (!name || !date) return toast.error("Name and date are required");
    setList((l) => [...l, { id: `H-${Math.random().toString(36).slice(2, 7)}`, name, date, region: region || "All", optional }]);
    toast.success(`${name} added to holidays`);
    setOpen(false); setName(""); setDate(""); setRegion(""); setOptional(false);
  };
              ...
              {list.sort((a, b) => a.date.localeCompare(b.date)).map((h) => (
                <li key={h.id} className="flex items-center justify-between py-3 first:pt-0 last:pb-0">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold">{h.name}{h.optional && <span className="ml-2 rounded-full bg-muted px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Optional</span>}</p>
                      <p className="text-xs text-muted-foreground inline-flex items-center gap-1"><MapPin className="h-3 w-3" /> {h.region}</p>
                    </div>
                  </div>
                  <p className="text-sm font-medium">{new Date(h.date).toLocaleDateString(undefined, { weekday: "short", day: "2-digit", month: "short", year: "numeric" })}</p>
                </li>
              ))}
            </ul>
          </DataCard>
              ...
              {Array.from({ length: days }).map((_, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    {h && <div className="mt-1 line-clamp-2 text-[10px] font-medium leading-tight text-primary">{h.name}</div>}
                  </div>
                );
              })}
            </div>
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button className="rounded-full" onClick={add}>Add</Button>
          </>
        }
      >
        <div className="space-y-3">
          <div>
            ...
            <Input id="hn" value={name} onChange={(e) => setName(e.target.value)} className="mt-1.5" placeholder="e.g. Diwali" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label htmlFor="hd">Date</Label>
              <Input id="hd" type="date" value={date} onChange={(e) => setDate(e.target.value)} className="mt-1.5" />
            </div>
            <div>
              <Label htmlFor="hr">Region</Label>
              <Input id="hr" value={region} onChange={(e) => setRegion(e.target.value)} className="mt-1.5" placeholder="IN / US / All" />
            </div>
          </div>
          <div className="flex items-center justify-between rounded-xl bg-muted/40 px-3 py-2.5">
            <div>
              <p className="text-sm font-medium">Optional holiday</p>
              <p className="text-xs text-muted-foreground">Employees can choose whether to take it.</p>
            </div>
