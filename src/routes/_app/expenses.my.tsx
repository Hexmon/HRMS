...
function MyExpenses() {
  ...
  const cols: Column<ExpenseTicket>[] = [
    ...
    },
    {
      ...
      render: (r) => (
        <span className="text-xs text-muted-foreground">
          {r.expenseType === "project" ? "Project" : "Sales / Pre-sales"} · {r.subType}
        </span>
      ),
    },
    {
      key: "title",
      header: "Task",
      render: (r) => <span className="text-sm font-medium">{r.taskTitle}</span>,
    },
    {
      key: "amount",
      header: "Amount",
    ...
    },
    {
      key: "stage",
      header: "Stage",
      render: (r) => <span className="text-xs capitalize text-muted-foreground">{r.stage}</span>,
    },
