...
function MyTicketsScreen() {
  ...
  const rows = useMemo(
    () => tickets.filter((t) => t.raisedBy === user?.name).sort((a, b) => (a.createdAt < b.createdAt ? 1 : -1)),
    [tickets, user],
  );
  ...
  const cols: Column<Ticket>[] = [
    ...
    { key: "assignee", header: "Assigned", render: (t) => <span className="text-sm">{t.assignee ?? "—"}</span> },
    { key: "sla", header: "SLA", render: (t) => <SlaBadge ticket={t} /> },
    { key: "created", header: "Created", render: (t) => <span className="text-xs text-muted-foreground">{fmtRelative(t.createdAt)}</span> },
    { key: "updated", header: "Last update", render: (t) => <span className="text-xs text-muted-foreground">{fmtRelative(t.updatedAt)}</span> },
  ];
  ...
  return (
    <DataTable
      ...
      rowActions={(t) => [{ label: "Open ticket", onClick: () => navigate({ to: "/helpdesk/$id", params: { id: t.id } }) }]}
    />
  );
}
