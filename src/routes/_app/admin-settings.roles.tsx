...
import {
  useAdminSettings, PERMISSION_GROUPS, PERMISSION_ACTIONS,
  type PermissionAction, type PermissionGroup,
        ...
        <Card className="rounded-2xl border-border/60 p-5">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div className="space-y-1.5">
              ...
              <Input value={active.label} onChange={(e) => updateRoleMeta(active.key, { label: e.target.value })} />
            </div>
            <div className="flex items-center gap-3">
              <span className="grid h-10 w-10 place-items-center rounded-xl bg-primary-soft text-primary"><Users className="h-5 w-5" /></span>
              <div>
            ...
            <div className="space-y-1.5 md:col-span-2">
              ...
              <Textarea rows={2} value={active.description} onChange={(e) => updateRoleMeta(active.key, { description: e.target.value })} />
            </div>
          </div>
        </Card>
        ...
        <Card className="rounded-2xl border-border/60 p-0">
          <div className="flex items-center justify-between border-b p-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Toggle granular access by module and action.</p>
            </div>
            <Badge variant="outline" className="font-mono text-[10px]">{active.key}</Badge>
          </div>
            ...
            <table className="w-full min-w-[760px] text-sm">
              <thead>
                <tr className="border-b bg-muted/30 text-left text-[11px] uppercase tracking-wider text-muted-foreground">
                  ...
                  {PERMISSION_ACTIONS.map((a) => <th key={a} className="px-3 py-2.5 text-center font-semibold capitalize">{a}</th>)}
                  ...
                  return (
                    <tr key={g} className="border-b last:border-0 hover:bg-accent/40">
                      ...
                      {PERMISSION_ACTIONS.map((a) => (
                        <td key={a} className="px-3 py-2.5 text-center">
                          <Checkbox
                            ...
                            onCheckedChange={() => togglePermission(active.key, g as PermissionGroup, a as PermissionAction)}
                      ...
                      <td className="px-3 py-2.5 text-center">
                        <Checkbox
                          checked={allOn}
                          onCheckedChange={(v) => toggleAllForGroup(active.key, g as PermissionGroup, !!v)}
                          disabled={active.key === "main_admin"}
                        />
                      </td>
                    </tr>
          ...
          <div className="flex items-center justify-end gap-2 border-t p-4">
            <Button variant="ghost" onClick={() => setActiveKey(null)}>Done</Button>
            <Button
              style={{ background: "var(--gradient-primary)" }}
              className="text-primary-foreground"
              onClick={() => { toast.success("Role permissions saved"); setActiveKey(null); }}
            >
              Save & close
            </Button>
          </div>
    ...
    <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
      {roles.map((r) => {
        const enabled = PERMISSION_GROUPS.reduce(
          (acc, g) => acc + PERMISSION_ACTIONS.filter((a) => r.matrix[g][a]).length, 0,
        );
          ...
          <button key={r.key} onClick={() => setActiveKey(r.key)} className="text-left">
            <Card className={cn(
              ...
              <div className="mt-3 flex items-center justify-between text-xs">
                <span className="text-muted-foreground"><span className="font-semibold text-foreground">{r.assignedUsers}</span> users</span>
                <span className="text-muted-foreground"><span className="font-semibold text-foreground">{pct}%</span> permissions</span>
              </div>
