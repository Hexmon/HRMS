...
function AgentQueueScreen() {
  ...
  const cols: Column<Ticket>[] = [
    ...
    { key: "requester", header: "Requester", render: (t) => <div className="text-xs"><p className="font-medium text-foreground">{t.raisedBy}</p><p className="text-muted-foreground">{t.raisedByDept ?? "—"}</p></div> },
    ...
    { key: "age", header: "Age", render: (t) => <span className="text-xs text-muted-foreground">{fmtRelative(t.createdAt)}</span> },
  ];
  ...
  const Table = ({ rows }: { rows: Ticket[] }) => (
    <DataTable
      ...
      rowActions={(t) => [{ label: "Open", onClick: () => navigate({ to: "/helpdesk/$id", params: { id: t.id } }) }]}
    />
  );
  ...
  return (
    <Tabs defaultValue="mine">
      ...
      <TabsContent value="new" className="mt-4"><Table rows={groups.new} /></TabsContent>
      <TabsContent value="mine" className="mt-4"><Table rows={groups.mine} /></TabsContent>
      <TabsContent value="unassigned" className="mt-4"><Table rows={groups.unassigned} /></TabsContent>
      <TabsContent value="overdue" className="mt-4"><Table rows={groups.overdue} /></TabsContent>
      <TabsContent value="escalated" className="mt-4"><Table rows={groups.escalated} /></TabsContent>
      <TabsContent value="resolved" className="mt-4"><Table rows={groups.resolved} /></TabsContent>
      <TabsContent value="closed" className="mt-4"><Table rows={groups.closed} /></TabsContent>
    </Tabs>
  );
}
