...
export function workingDatesInclusive(
  start: string,
  end: string,
  workingWeek: string | null | undefined,
  holidayDates: ReadonlySet<string> = new Set()
  ...
  while (cursor <= to) {
    ...
    if (isWorkingDate(date, workingWeek, holidayDates)) {
      dates.push(date);
    }
...
export function workdaysInclusive(
  ...
  return workingDatesInclusive(start, end, workingWeek, holidayDates).length;
}
