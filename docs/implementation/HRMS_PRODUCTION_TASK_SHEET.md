...
| HRMS-QA-001 | QA | Enterprise QA artifacts | Generate tester-ready internal/client checklists and sheets | Completed | `qa/*`, `qa/scripts/generate_testing_artifacts.py` | XLSX/DOCX generated with stdlib script | P0 | QA Execution | External HRMS.xlsx patch is provided because safe spreadsheet library is unavailable |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| HRMS-QA-UPGRADE-001 | Full software QA workbook | Rebuilt the QA workbook into a first-cycle full-product manual QA plan with P0/P1/P2 test tabs, sprint plan, coverage matrix, role matrix, business traceability, test data, defect log, blocked questions, future scope, and signoff tabs | Completed | `qa/TESTING_TEST_CASES.xlsx` | 3 sprints, 7 days each, 48 SP capacity; Sprint 1 = 40.5 SP, Sprint 2 = 39.5 SP, Sprint 3 = 44.5 SP | `python3 qa/scripts/validate_qa_artifacts.py` passed | Covers full product areas while keeping P0 limited to release/UAT gate flows |
...
| HRMS-QA-UPGRADE-004 | Tester manual | Rewrote tester run book with execution order, defect logging rules, request ID capture, macOS/Ubuntu/Windows setup guidance from repo evidence, Cloudinary/API-mode rules, role login guidance, FAQ, and signoff conditions | Completed | `qa/TESTER_RUN_BOOK.md` | Supports all 3 QA sprints | QA artifact validator confirmed OS setup sections are present | Setup commands are marked Needs Confirmation where repo evidence is insufficient |
| HRMS-QA-UPGRADE-005 | Client/internal QA summaries | Regenerated client-safe checklist, internal checklist, and client DOCX summary from the upgraded workbook scope | Completed | `qa/TESTING_CHECKLIST_CLIENT.md`, `qa/TESTING_CHECKLIST_INTERNAL.md`, `qa/TESTING_CHECKLIST_CLIENT.docx` | Summary docs point testers to workbook execution order | DOCX/XLSX generated by stdlib script; validator passed workbook and docs presence checks | Client document avoids internal code paths |
