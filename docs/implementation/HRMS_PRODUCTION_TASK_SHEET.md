...
| Asset workflow backend | Implemented for asset requests, approvals, cancellations, acknowledgements, maintenance records, vendor list/create/update, recovery queue, and recovery settlement |
| Asset workflow frontend | Integrated in `hrms-client` asset store/API mode for visible asset request, acknowledgement, maintenance, vendor CRUD, and recovery settlement flows |
...
| Phase 6 verification guardrails | Hardened backend implementation and scalability checks now cover the full module set, 232-operation contract state, zero planned API backlog, synced frontend contract JSON, critical queue/report indexes, and frontend production API/mock fallback config |
...
| Project Manager, Main Admin, Finance | `/projects`, `/projects/$id`, `/team-utilization`, project reports | Backend API in API mode; project document upload/download uses existing Documents API | `/api/v1/projects*`, `/api/v1/team-utilization/summary`, `/api/v1/reports/projects/summary`, `/api/v1/documents*` | Project CRUD/status/member/allocation/milestone actions use backend; editable API-backed project detail can upload compressed document files and open generated download URLs | Project/utilization route smoke covered; upload browser coverage still needs expansion | Production-ready with P1 project report-depth/browser-upload coverage gap |
...
| Asset Admin, Employee | `/assets*`, asset reports | Backend API in API mode | `/api/v1/assets*`, `/api/v1/reports/assets/summary` | Requests/approvals/acknowledgement/maintenance, vendor create/update, and recovery settlement use backend | API integration covered; route smoke covered; deeper browser approval/recovery coverage still needs expansion | Production-ready with P1 browser-flow coverage gap |
...
| Admin, Auditor, Finance, HR | `/reports*` | Backend API in API mode with generated CSV document handoff for supported exports | `/api/v1/reports*`, `/api/v1/documents/{id}/download-url` | CSV exports generate backend documents; XLSX remains queued/pending renderer | Browser export download handoff covered route-wide | Production-ready for CSV; P1 XLSX/scheduling/retention gap |
...
| Deferred coverage | Browser-level frontend e2e, XLSX rendering, scheduled exports, retention policy automation, and route-wide browser export-download assertions remain Phase 6 hardening work | The new smoke test is API-level coverage, not a replacement for Playwright/browser coverage |
...
| Upload journey verification | Expanded the Documents integration suite to use real multipart payloads for PDF, image, and text/general file uploads, plus expense and EMS wrapper uploads | Verified upload, object-storage stat, list visibility, EMS list wrapper, download-url handoff, expense document attachment, image Cloudinary optimization metadata, and PDF compression fail-open metadata |
...
| 6 | EMS document upload picker | Replace fake Upload/Replace UI with selected-file metadata upload | Backend wrapper already complete | Completed for `/ems/documents` global Upload and per-document Replace actions in API mode | `pnpm exec tsc -p tsconfig.json --noEmit`, `pnpm lint`, route guard, route coverage, `pnpm build` | Passed. Frontend lint keeps 39 existing Fast Refresh warnings; build keeps chunk-size/Wrangler log warnings but exits 0. | `hrms-client/src/routes/_app/ems.documents.tsx` | `feat(ems): add document upload picker` |
...
| P1 | Warranty automation/alerts remain planned for asset operational hardening; vendor create/update and recovery settlement are now API-backed |
| P1 | Project-specific report depth remains planned; project document upload/download is now API-backed on project detail |
...
- `pnpm exec vitest run --project integration src/modules/documents/__tests__/documents.integration.test.ts`: passed, 3 tests covering restricted access plus multipart PDF/image/text upload, expense document upload, EMS document upload, list/download handoff, storage stat, and compression metadata
...
- `pnpm api:docs:generate`: passed with escalation due `tsx` IPC sandboxing; generated 232 operation frontend contract after asset vendor/recovery hardening
...
- `pnpm api:implemented-route-guard`: passed, 59 files against 204 paths
...
- Asset vendor create/update is implemented for the visible `/assets/warranty` vendor workflow; delete/deactivation beyond status changes and automated warranty alerts remain deferred hardening.
...
2. Backend OpenAPI now has 232 operations across 204 paths; planned operations remaining are 0.
...
4. Employee/core CSV, report CSV, and Attendance/Leave-WFH CSV/JSON exports now generate backend documents and return `download_document_id`; employee import parsing, XLSX workbook rendering, scheduled exports, and retention cleanup remain production hardening.
5. Next roadmap scope: continue Phase 6 with approval/upload browser coverage, warranty automation/alerts, project-specific report depth, import/XLSX/scheduled-export/retention hardening, dynamic RBAC assignment/enforcement, and release-readiness reporting.
