...
| Phase 6 provider-based document storage | Restored MinIO as the local/demo object-storage backend and kept Cloudinary as an env-selectable provider for production or later migration. The upload/download path remains backend-owned, with no object-storage credentials exposed to the frontend. |
...
| Runtime/deployment wiring | Added PDF compression and object-storage provider env keys to tracked examples and Docker compose runtime anchors, restored MinIO for local/QA demo stacks, and installed `ghostscript` in the backend Docker image | Local/test/QA use MinIO by default and do not require Cloudinary credentials; production examples keep Cloudinary as the default and require real Cloudinary credentials with `CLOUDINARY_MOCK_UPLOADS=false` |
...
| 6 | Object-storage provider switch | Restore MinIO for local/demo uploads and keep Cloudinary switchable by env | Completed for runtime config, datastore wiring, document upload/download, generated exports, release verification scripts, Docker dev/QA MinIO service, env examples, README/API docs, and provider-neutral tests | N/A | `pnpm typecheck`, `pnpm build`, `pnpm lint`, `pnpm api:docs:generate`, `pnpm api:docs:verify`, `pnpm test:unit`, `HRMS_ENV_FILE=.env.test.example pnpm test:contracts`, `HRMS_ENV_FILE=.env.test.example pnpm exec vitest run --project integration src/modules/documents/__tests__/documents.integration.test.ts`, `HRMS_ENV_FILE=.env.test.example pnpm db:verify:no-cross-schema-fks`, `HRMS_ENV_FILE=.env.test.example HRMS_REPORT_DIR=/private/tmp/hrms-minio-smoke pnpm release:verify:services`, Docker dev/QA/prod compose config parse | Passed. Initial non-escalated tsx and local DB runs were sandbox-blocked; escalated reruns passed. MinIO direct put/stat/delete and full document upload/download journey passed against local containers. | `src/platform/object-storage.ts`, config/decorator/app/store wiring, document/generated export services, verification scripts, Docker compose files, env examples, README, API docs, task sheets, tests | `feat(storage): restore MinIO provider for local uploads` |
...
- `pnpm api:docs:generate`: passed with escalation due `tsx` IPC sandboxing; generated 233 operation frontend contract after asset warranty-alert hardening
...
2. Backend OpenAPI now has 233 operations across 205 paths; planned operations remaining are 0.
