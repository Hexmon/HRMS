...
| Project Manager, Main Admin, Finance | `/projects`, `/projects/$id`, `/team-utilization`, project reports | Backend API in API mode; document upload/attach UX still absent on project detail | `/api/v1/projects*`, `/api/v1/team-utilization/summary`, `/api/v1/reports/projects/summary` | Project CRUD/status/member/allocation/milestone actions use backend; project documents are read-only list | Project/utilization route smoke covered | Production-ready with P1 project document attach/report-depth gap |
...
5. Next roadmap scope: continue Phase 6 with approval/upload browser coverage, warranty automation/alerts, project document attach UX, import/XLSX/scheduled-export/retention hardening, dynamic RBAC assignment/enforcement, and release-readiness reporting.
