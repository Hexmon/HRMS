...
- First-pass visible frontend API coverage is complete in this contract pack. Remaining work is production hardening and explicitly scoped future contracts; Admin Security Settings now persists and enforces the visible password/session/rate basics, and EMS admin workflow actions are API-backed.

## API Count Summary

Current documented backend contract: **229 operations** in `openapi.json` after Phase 6 EMS admin workflow hardening.

- **227** operations are under `/api/v1/**`.
...
| --- | ---: | --- |
| Existing APIs ready to integrate as-is | 229 | Present in `openapi.json` and usable through the generated frontend client without path or workflow changes. |
...
| Target contract size after additions | 229 | First-pass visible frontend contract plus EMS admin workflow hardening is complete at 229 operations. |
...
| EMS | 0 | Primary profile, profile requests, employee document wrappers, letters, policies, generic requests, HR queues, and visible HR admin workflow actions are implemented. |
...
| EMS                        | `/ems/*`                                                                                                    | Profile, profile update requests, HR profile queue/decisions, employee document wrappers, generic employee requests, HR request queue/decisions, HR letters, policy acknowledgements, onboarding/exit checklists, probation decisions, policy update/publish workflow, and Documents APIs for download/verify. | Provider-generated letter PDFs, automated onboarding task orchestration, and deeper browser decision-flow coverage remain production hardening.                                                 |
