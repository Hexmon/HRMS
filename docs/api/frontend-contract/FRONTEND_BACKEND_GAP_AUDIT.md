...
- Full frontend coverage requires additional API groups for EMS admin/document wrappers, attendance daily/queue/export gaps, leave/WFH export jobs, employee import/export/audit history, admin security settings, and production hardening.

## API Count Summary

Current documented backend contract: **206 operations** in `openapi.json` after Phase 5 non-expense Reports API completion.

- **204** operations are under `/api/v1/**`.
...
| --- | ---: | --- |
| Existing APIs ready to integrate as-is | 206 | Present in `openapi.json` and usable through the generated frontend client without path or workflow changes. |
...
| New APIs remaining to add | 11 | Remaining first-pass count needed after Phase 5 non-expense Reports API completion. |
| Target contract size after additions | 217 | `206 current + 11 remaining`; non-expense Reports APIs are now implemented. |
...
| **Total remaining** | **11** | Remaining operation count for full visible frontend coverage after Phase 5 non-expense Reports. |
