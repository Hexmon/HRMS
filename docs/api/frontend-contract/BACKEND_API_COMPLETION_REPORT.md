...
| --- | ---: | --- | --- |
| Implemented APIs ready to integrate | 233 | Use generated client from `openapi.json`. | Keep behavior stable and fix bugs only. |
...
| Target implemented contract after completion | 233 | Regenerate frontend client after future backend phases land. | First-pass visible frontend contract plus asset vendor/recovery/warranty-alert hardening is complete at 233 operations. |
...
| Assets | 23 | Inventory, detail, warranty alerts, assignment/return, QR scan, license lifecycle, employee termination event, requests, acknowledgements, maintenance, vendor list/create/update, recovery queue, and recovery settlement. |
...
| Expenses / Requester | 10 | Create/list/detail/update placeholder/submit/timeline, metadata, dashboard summary, withdraw, and clarifications. |
...
| Method | Planned path | Frontend route/screen | Purpose and business behavior | Auth/persona | Inputs | Success response | Errors/OCC/rate notes | State |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| POST | `/api/v1/core/users` | `/employees/new` | Create employee profile and optional login setup action. | HR/Admin only. | employee_code, email, full_name, department_id, designation_id, manager_user_id, roles, login_enabled | user detail with version and onboarding status | 400 validation, 403 role, 409 duplicate employee_code/email. | Implemented in Phase 3 |
| PATCH | `/api/v1/core/users/{id}` | `/employees/:id` | Update employee profile, reporting line, and lifecycle fields. | HR/Admin only. | Path id; body profile/work fields plus expected_version | updated user detail with version | 409 stale version; hierarchy cycle blocked. | Implemented in Phase 3 |
| POST | `/api/v1/core/users/{id}/activate` | `/employees/:id` | Reactivate employee profile. | HR/Admin only. | Path id; expected_version, remarks | user detail with status/version | 409 invalid status or stale version. | Implemented in Phase 3 |
| POST | `/api/v1/core/users/{id}/deactivate` | `/employees/:id` | Deactivate, suspend, or terminate employee and emit offboarding event. | HR/Admin only. | Path id; reason, effective_date, status, expected_version | user detail with login disabled and version | 409 if already in requested status; outbox event emitted. | Implemented in Phase 3 |
...
| Method | Planned path | Frontend route/screen | Purpose and business behavior | Auth/persona | Inputs | Success response | Errors/OCC/rate notes | State |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| GET | `/api/v1/ems/profile/me` | `/ems/profile` | Return self-service employee profile. | Authenticated employee. | None | profile, reporting_line, emergency_contacts, summaries | 403 inactive employee; no cross-user data. | Implemented in Phase 3 EMS |
| PATCH | `/api/v1/ems/profile/me` | `/ems/profile` | Update allowed self-service profile fields directly where policy permits. | Authenticated employee. | editable profile fields, expected_version | profile, version, pending_approval flag | 409 stale version; restricted fields require change request. | Implemented in Phase 3 EMS |
| POST | `/api/v1/ems/profile-change-requests` | `/ems/profile/change-request` | Submit one profile field change request for HR approval. | Authenticated employee. | field_key, field_label optional, new_value, reason, supporting_document_ids[] | request_id, status, request | 400 unsupported field, 409 duplicate pending request. | Implemented in Phase 3 EMS |
...
| POST | `/api/v1/ems/policies/{id}/acknowledge` | `/ems/policies/:id` | Acknowledge company policy. | Assigned employee. | Path id; expected_version | policy, acknowledgement status, version | 409 already acknowledged or stale version. | Implemented in Phase 3 EMS |
| POST | `/api/v1/ems/requests` | `/ems/requests` | Create generic employee service request. | Authenticated employee. | request_type, subject, description, document_ids[] | request_id, request, status | 400 invalid type; route to HR queue. | Implemented in Phase 3 EMS |
| GET | `/api/v1/ems/requests/my` | `/ems/requests` | List own generic employee service requests. | Authenticated employee. | page, page_size, type, status | items[], pagination | Shared errors; this endpoint was added after UI route audit. | Implemented in Phase 3 EMS |
| GET | `/api/v1/ems/requests/queue/hr` | `/ems/hr/requests` | HR queue for generic employee requests. | HR/Admin only. | page, page_size, type, status, department_id, user_id | items[], pagination, queue_counts | 403 non-HR; supports assignment later if needed. | Implemented in Phase 3 EMS |

### Attendance (12 implemented APIs, 0 planned APIs)
...
| GET | `/api/v1/attendance/calendar/daily` | `/attendance/daily` | Return day-level attendance status for selected users. | Employee self, manager team, HR/Admin. | date, user_id optional, team_id optional | day records, exceptions, regularization flags | 403 out-of-scope team/user. | Implemented in Phase 5 Attendance backlog |
...
| GET | `/api/v1/attendance/regularizations/queue/manager` | `/attendance/approvals` | Manager queue for regularization decisions. | Manager or configured backup; HR/Admin monitor. | page, page_size, status, department_id | items[], pagination, queue_counts | Self-processing blocked; 403 if not manager. | Implemented in Phase 5 Attendance backlog |
...
| Method | Planned path | Frontend route/screen | Purpose and business behavior | Auth/persona | Inputs | Success response | Errors/OCC/rate notes | State |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| GET | `/api/v1/timesheets/projects/summary` | `/timesheet/projects` | Return project-level timesheet rollups. | Employee self, manager team, PM, HR/Admin by scope. | date_range, project_id, user_id, page, page_size | items[], totals, pagination | 403 out-of-scope; paginated. | Implemented in Phase 4 Timesheets |
| GET | `/api/v1/timesheets/missing-submissions` | `/timesheet/missing` | Return missing submission list for cycles. | Manager/HR/Admin; self can see own missing cycles. | cycle_start, cycle_end, manager_user_id, page, page_size | items[], pagination, reminder_state | 403 scope; 429 if coupled with reminders later. | Implemented in Phase 4 Timesheets |
| GET | `/api/v1/timesheets/productivity-summary` | `/timesheet/productivity` | Return productivity summary cards and charts. | Employee self, manager team, HR/Admin. | date_range, user_id, project_id, group_by | cards, series[], breakdown[] | Shared errors; no client-side all-data aggregation. | Implemented in Phase 4 Timesheets |
| GET | `/api/v1/timesheets/submissions/{id}` | `/timesheet/submissions/:id` | Return submission detail with segments and decision history. | Owner, approver, HR/Admin/auditor. | Path id | submission, segments[], workflow_history[], version | 403 out-of-scope; 404 unknown. | Implemented in Phase 4 Timesheets |
| GET | `/api/v1/timesheets/selectors` | `/timesheet/forms` | Return projects, tasks, cycles, approvers, and workflow metadata for forms. | Authenticated user; scoped to assignments. | include, date optional | projects[], tasks[], cycles[], approvers[], rules | Shared errors; cache short-lived. | Implemented in Phase 4 Timesheets |
...
### Expenses / Finance Additions (4 implemented APIs)

| Method | Planned path | Frontend route/screen | Purpose and business behavior | Auth/persona | Inputs | Success response | Errors/OCC/rate notes | State |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| GET | `/api/v1/expenses/metadata` | `/expenses/forms` | Return categories, currencies, policy hints, and document type selectors. | Authenticated employee. | include, date optional | categories[], currencies[], policy_hints, document_types[] | Shared errors; cache short-lived and refetch after policy changes. | Implemented in Phase 4 Expenses |
| GET | `/api/v1/expenses/dashboard-summary` | `/expenses` | Return role-aware expense dashboard cards. | Requester, manager, finance, admin by scope. | date_range, status optional | cards, queue_counts, aging, totals | 403 for unavailable role; money as strings. | Implemented in Phase 4 Expenses |
...
| GET | `/api/v1/assets/warranty-alerts` | `/assets/warranty` | List expired and soon-expiring warranties using the Admin asset policy alert window. | Asset admin/Admin. | page, page_size, window_days, include_expired | items[], pagination, alert_window_days, counts | 403 non-asset-admin; policy-driven alert window. | Implemented in Phase 6 Asset hardening |
...
| GET | `/api/v1/admin/master-data/departments` | `/admin-settings/master-data` | List departments. | Admin/HR. | page, page_size, active_only, search | items[], pagination | Shared errors. | Implemented in Phase 5 Admin master data |
| POST | `/api/v1/admin/master-data/departments` | `/admin-settings/master-data` | Create department. | Admin/HR. | name, code, parent_id optional | department, version | 409 duplicate code. | Implemented in Phase 5 Admin master data |
| PATCH | `/api/v1/admin/master-data/departments/{id}` | `/admin-settings/master-data` | Update department. | Admin/HR. | fields, expected_version | department, version | 409 stale version/active references. | Implemented in Phase 5 Admin master data |
| GET | `/api/v1/admin/master-data/designations` | `/admin-settings/master-data` | List designations. | Admin/HR. | page, page_size, active_only, search | items[], pagination | Shared errors. | Implemented in Phase 5 Admin master data |
| POST | `/api/v1/admin/master-data/designations` | `/admin-settings/master-data` | Create designation. | Admin/HR. | name, code, level, department_id optional | designation, version | 409 duplicate code. | Implemented in Phase 5 Admin master data |
| PATCH | `/api/v1/admin/master-data/designations/{id}` | `/admin-settings/master-data` | Update designation. | Admin/HR. | fields, expected_version | designation, version | 409 stale version/active references. | Implemented in Phase 5 Admin master data |
| GET | `/api/v1/admin/rbac/roles` | `/admin-settings/rbac` | List roles. | Admin/RBAC manager. | page, page_size, active_only | items[], pagination | 403 non-admin. | Implemented in Phase 5 Admin RBAC |
| POST | `/api/v1/admin/rbac/roles` | `/admin-settings/rbac` | Create role. | Admin/RBAC manager. | name, description, permission_ids[] | role, permissions, version | 409 duplicate role. | Implemented in Phase 5 Admin RBAC |
...
| GET | `/api/v1/admin/audit-log` | `/admin-settings/audit` | Admin settings audit log. | Admin/auditor. | page, page_size, module, actor_user_id, date_range | items[], pagination | Redact secrets; 403 non-auditor/admin. | Implemented in Phase 5 Admin audit log |
